...
This page contains details about the `make_tests.sh` script features for writing new tests in the testsuite.

The testsuite is hosted at https://github.com/gpac/testsuite

The `scripts/` directory contains the existing tests and can be used as examples for writing new tests as well as example usage of some specific GPAC features.
...
Each call generates:
- a log file called `TESTNAME-single-logs.txt` containing test name, command line and all stderr of the test. You should not use file logging of GPAC ( `-lf` or `-logs` options) in your command line.
- a file called `TESTNAME-single-passed.xml` containing the statistics of the test, as retrieved by GNU time/gtime.
